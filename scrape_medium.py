#!/usr/bin/env python3
"""Scrape Medium articles using undetected-chromedriver.
Must run with: xvfb-run --auto-servernum python3 -u scrape_medium.py"""
...
# Also need the 2 retrospective posts - find URLs from their QMDs
import glob
...
def clean_medium_text(text):
    """Remove Medium UI chrome from article text."""
    lines = text.split('\n')
    clean = []
    for line in lines:
        s = line.strip()
        # Skip obvious Medium UI elements
        if s in ['Sign up', 'Sign in', 'Open in app', 'Member-only story', 
                 'Listen', 'Share', 'Follow', 'More from']:
            continue
        if s.startswith('·') and 'min read' in s:
            continue
...
def main():
    print(f"Starting Chrome for Medium scraping ({len(medium_posts)} articles)...", flush=True)
    ...
    try:
        for i, (slug, url) in enumerate(medium_posts.items()):
            ...
            try:
                driver.get(url)
                time.sleep(5)  # Medium is JS-heavy
                ...
                for sel in ['article', '[id*="root"] article', '.meteredContent', 'section']:
                    try:
                ...
                if not content:
                    # Fallback: get full body text
                    try:
                        content = driver.find_element(By.TAG_NAME, "body").text.strip()
                    except:
                        content = ""
                
                content = clean_medium_text(content)
    ...
    print(f"\n{'='*50}", flush=True)
    print(f"DONE: {ok}/{len(medium_posts)} articles scraped", flush=True)
    total_chars = sum(len(v) for v in results.values())
    print(f"Total content: {total_chars:,} chars", flush=True)
